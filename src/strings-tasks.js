...
function removeFirstOccurrences(str, value) {
  const indexOfValueInStr = str.indexOf(value);
  if (!value.length || indexOfValueInStr === -1) return str;

  return (
    str.slice(0, indexOfValueInStr) +
    str.slice(indexOfValueInStr + value.length, str.length)
  );
}
...
function removeLastOccurrences(str, value) {
  const indexOfValueInStr = str.lastIndexOf(value);
  if (!value.length || indexOfValueInStr === -1) return str;
...
function sumOfCodes(str) {
  return str ? str.split('').reduce((s, char) => char.charCodeAt(0) + s, 0) : 0;
}
  ...
  return str
    ...
    .map((item) => {
      if (alphabet.includes(item)) {
        return codeStr[alphabet.indexOf(item)];
      }
      return item;
    })
    .join('');
